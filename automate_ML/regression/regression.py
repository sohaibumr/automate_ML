	...
	def predict(self, prediction_data='test', unknown_data=None, save_csv=False, file_name= 'predicted_data'):
		...
		if prediction_data == 'test':
			...
			if save_csv:
				...
				df.to_csv(file_name + '_' + self.timenow + '.csv', index_label='index')
			else:
				pass
...
##############################################******Visualization********###############################################
	...
	def plot_scatter(self, plot_for="test", facecolor='red', alpha=0.5, marker='o', xlabel='True', ylabel='Predicted', title='Regression_plot',
		...
		if plot_for == 'train':
			ax = sns.regplot(x=y_train, y=y_train_predicted, marker=marker)
			plt.xlabel(xlabel)
			plt.ylabel(ylabel)
			plt.title(title)
			ax.text(max(y_train/2), min(y_train*2.5), ('R2 =', round(metrics.r2_score(y_train, y_train_predicted) , 3)), bbox=dict(facecolor=facecolor, alpha=alpha))
			plt.show()
		elif plot_for == 'test':
			ax  = sns.regplot(x=y_test, y=y_test_predicted, marker=marker)
			plt.xlabel(xlabel)
			plt.ylabel(ylabel)
			plt.title(title)
			ax.text(max(y_test / 2), min(y_test * 2.5), ('R2 =', round(metrics.r2_score(y_test, y_test_predicted) , 3)),
	...
	def save_data(self, filename=None, verbosity=0):
		...
		if filename is None:
			save_file = open("results" + self.timenow + '.txt', "w")
